...
// Create new user (admin only) - FIXED VERSION
 // Updated createUser function - Remove username uniqueness check
...
//     // Check ONLY if email already exists (REMOVED username checking)
//     console.log('🔍 Checking for existing email only...');
//     const existingUserResult = await query(
//       'SELECT id, email FROM users WHERE email = $1',
//       [email]
...
//     // Create user - Username duplicates are allowed (including "Admin")
//     console.log('✏️ Creating user in database...');
//     console.log(`ℹ️ Username "${username}" is allowed (duplicates permitted)`);
    
//     const newUserResult = await query(
//       `INSERT INTO users (name, email, username, password, department, role)
//        VALUES ($1, $2, $3, $4, $5, $6)
//        RETURNING id, name, email, username, department, role, created_at`,
//       [name, email, username, hashedPassword, department, role]
//     );
...
//     if (error.code === '23505') { // Unique violation
//       // Only check for email constraint violation since username duplicates are allowed
//       if (error.constraint && error.constraint.includes('email')) {
//         res.status(409).json({
//           success: false,
//           message: 'Email already exists'
//         });
//       } else if (error.constraint && error.constraint.includes('username')) {
//         // This should not happen after removing the constraint, but just in case
//         console.log('⚠️ Username constraint still exists in database');
//         res.status(500).json({
//           success: false,
//           message: 'Database configuration error - username constraint should be removed'
